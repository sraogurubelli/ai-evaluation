...
**SDK:** See [SDK](sdk.md) for defining an Eval (using the `Experiment` class), `HTTPAdapter`, `DeepDiffScorer`, `load_jsonl_dataset`. For index-CSV + CSV: `samples_sdk/consumers/devops` (run with `PYTHONPATH=.` from repo root).
...
- **PostgreSQL** is needed for task storage and run history. Options: install Docker and run `task db-up`; or use an existing PostgreSQL and set `DATABASE_URL` in `.env`.
