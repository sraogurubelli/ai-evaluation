...
- **Cost placement** — Optional cost/token data can live in:
  ...
  - **ExperimentRun.metadata** — Run-level: `aggregate_metrics: { accuracy, cost, latency_sec, input_tokens, output_tokens }`.
...
3. **Run-level aggregates** — Call `aieval.tracing.aggregates.enrich_run_aggregate_metrics(run, tracing_adapter)` to compute `aggregate_metrics` from scores and tracing and set `ExperimentRun.metadata["aggregate_metrics"]`.
