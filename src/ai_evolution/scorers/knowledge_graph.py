...
class KnowledgeGraphQualityScorer(Scorer):
    ...
    def _evaluate_kg_quality(
        ...
        if exp_count == 0:
            ...
            scores["insight_count_match"] = max(
        ...
        scores["query_validity"] = (
            valid_queries / max(len(gen_queries), 1) if gen_queries else 0.0
        )
        ...
        insights_with_severity = sum(
            1 for insight in gen_insights if insight.get("severity")
        )
        scores["has_severity"] = (
