    ...
    def check(
        ...
        for score_name, threshold in self.score_thresholds.items():
            ...
            if score_value is None:
                ...
                reasons.append(
                    f"Score '{score_name}' ({score_value:.4f}) below threshold ({threshold:.4f})"
                )
            else:
                score_checks[score_name] = {
                    "passed": True,
    ...
    def generate_report(self, check_result: dict[str, Any]) -> str:
        ...
        if check_result["score_checks"]:
            ...
            for score_name, check in check_result["score_checks"].items():
                ...
                if check["passed"]:
                    lines.append(
                        f"  {status} {score_name}: {check['value']:.4f} >= {check['threshold']:.4f}"
                    )
                else:
        ...
        if "regression_checks" in check_result:
            ...
            lines.append(
                f"  Regressions: {reg_checks['regression_count']} (max allowed: {reg_checks['max_allowed']})"
            )
