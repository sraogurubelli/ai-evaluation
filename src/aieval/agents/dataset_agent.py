...
class DatasetAgent(BaseEvaluationAgent):
    ...
    async def validate_dataset(
        ...
        **kwargs: Any,
        ...
        if schema or schema_file:
            ...
            try:
                schema_validation = validate_dataset_schema(
                    dataset, schema=schema, schema_file=schema_file
                )
                if not schema_validation["valid"]:
                    issues.append(
                        f"Schema validation failed: {schema_validation['invalid_count']} items invalid"
                    )
        ...
        self.logger.info(
            f"Dataset validation: {'valid' if is_valid else 'invalid'} ({len(issues)} issues)"
        )
    ...
    async def list_datasets(
        self, base_dir: str | None = None, **kwargs: Any
    ) -> list[dict[str, Any]]:
        """
