...
class FunctionAdapter(Adapter):
    ...
    def __init__(
        ...
        if not self.is_async and self.executor is None:
            self.executor = ThreadPoolExecutor(max_workers=4, thread_name_prefix="function-adapter")
    ...
    async def _call_sync(self, arguments: dict[str, Any]) -> Any:
        ...
        result = await loop.run_in_executor(self.executor, lambda: self.fn(**filtered_args))
        return result
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise FunctionAdapterError(f"Function {self.fn.__name__} raised error: {e}") from e
